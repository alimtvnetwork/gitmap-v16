...
After a successful release, if the repo's `remote.origin.url` contains the gitmap source repository prefix (`github.com/alimtvnetwork/gitmap-v13`), the CLI prints install one-liner commands for Windows (PowerShell `irm | iex`) and Linux/macOS (`curl | sh`). Non-gitmap repos are unaffected. Detection logic lives in `release/releaseinstallhint.go` with the prefix constant `GitmapRepoPrefix` in `constants_release.go`.
