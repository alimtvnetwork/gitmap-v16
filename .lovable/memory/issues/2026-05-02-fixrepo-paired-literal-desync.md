...
--- FAIL: TestBuildAuditNeedlesWidthCrossing
    replaceaudit_test.go:63: needle[6] = "gitmap-v12", want "gitmap-v13"
--- FAIL: TestRemoteSlugRegex
    replaceversionparse_test.go:57: regex "gitmap-v13" -> base="gitmap" num="13", want base="gitmap" num="12"
```
...
"gitmap-v13": {true, "gitmap", "12"},   // ← "12" was the captured num for v12
```
...
want := []string{
    ...
    "gitmap-v13", "gitmap/v12",  // ← only "gitmap-v13" got bumped
}
```

When fix-repo rewrites `gitmap-v12` → `gitmap-v13`, the sibling `"12"` / `12` remains, leaving an internally inconsistent test that fails on the next `go test` run.
...
3. The audit gives a precise diagnostic (`paired-literal desync at file:line: 'gitmap-v13' followed by sibling '12'`) instead of a downstream test failure.
