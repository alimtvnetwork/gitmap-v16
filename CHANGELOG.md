...
- Legacy-ref test data migrated to the current `gitmap-v11` repo namespace.
...
- New `gitmap/cmd/updatedebugwindows_source_test.go` parses
  ...
  - **`TestUpdateDebugWindowsHasFsutilLooseCall`** — the file imports
    `github.com/alimtvnetwork/gitmap-v11/gitmap/fsutil` AND contains at
...
- Installer scripts (`install.ps1`, `install.sh`, `uninstall.ps1`) embedded into the binary via `go:embed` (`gitmap/scripts/embed.go`), with HTTP fallback to `raw.githubusercontent.com/alimtvnetwork/gitmap-v11/main/gitmap/scripts/install.{ps1,sh}` if the embedded copy is missing.
...
- **README.md Go Report Card badge** now points at `github.com/alimtvnetwork/gitmap-v11/gitmap` (the real Go module path set in v3.27.0) instead of the repo root `github.com/alimtvnetwork/gitmap-v11`. The previous URL returned a 404 from the Go module proxy because there is no `go.mod` at the repo root — the module lives one directory down in `gitmap/`. Both the badge image and the click-through report link were updated.
...
- **Go Report Card now resolves the module instead of failing with `could not get latest module version from https://proxy.golang.org/github.com/user/gitmap/@latest`.** The card at https://goreportcard.com/report/github.com/alimtvnetwork/gitmap-v11/gitmap will start scoring the project for the first time after this version is pushed.
...
If left unfixed, anyone running `go install github.com/alimtvnetwork/gitmap-v11/gitmap@latest` would get a `module declares its path as: github.com/user/gitmap but was required as ...` error, and the proxy would refuse to serve the module to downstream tooling.

### Fix

Renamed `github.com/user/gitmap` → `github.com/alimtvnetwork/gitmap-v11/gitmap` across **404 files** in a single atomic sed pass. Also implicitly renamed the sister module `github.com/user/gitmap-updater` → `github.com/alimtvnetwork/gitmap-v11/gitmap-updater`, which lives at the same GitHub path.

Verified post-rename:
- `gitmap/go.mod` now reads `module github.com/alimtvnetwork/gitmap-v11/gitmap`.
- `gitmap-updater/go.mod` now reads `module github.com/alimtvnetwork/gitmap-v11/gitmap-updater`.
...
2. Once the new tag (`v3.27.0`) is pushed, visit https://goreportcard.com/report/github.com/alimtvnetwork/gitmap-v11/gitmap — first visit will trigger a fresh scan against the new module path.
...
## v3.16.0 — (2026-04-20) — uninstall-quick.ps1 multi-binary fix + repo rename to gitmap-v11
...
- **All `gitmap-v4` references → `gitmap-v11`** across the entire repo (45 files, 567 occurrences). Includes install/uninstall one-liners, Go installer constants, helptext, spec docs, post-mortems, the React landing page, and `.lovable/memory/**`.
...
- **Remote installer URLs** — Updated `constants_selfinstall.go` `SelfInstallRemotePwsh` and `SelfInstallRemoteBash` from `gitmap-v11` to `gitmap-v11`.
...
## v3.12.0 — (2026-04-20) — Pinned-version release snippet + gitmap-v11 rename
...
- **Unused-import build break** in `gitmap/store/migrations.go` — removed orphaned `"github.com/alimtvnetwork/gitmap-v11/gitmap/constants"` import left over from a prior refactor.
...
- After a successful release, if the repo's remote origin matches the gitmap source repository prefix (`github.com/alimtvnetwork/gitmap-v11`), the CLI now prints install one-liner commands for both Windows (PowerShell) and Linux/macOS (Bash).
...
- Repository renamed from `git-repo-navigator` to `gitmap-v11`; all URLs, scripts, and references updated.
