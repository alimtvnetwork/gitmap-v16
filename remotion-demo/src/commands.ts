...
export const COMMANDS: CommandDemo[] = [
  // 1) clone
  {
    ...
    cwd: "~/repos",
    lines: [
      cmd("~/repos", T("gitmap ", C.command), T("clone ", C.brandGold), T("https://github.com/alimtvnetwork/gitmap-v13", C.path)),
      out(T("→ Resolving remote… ", C.muted), T("OK", C.success)),
      out(T("→ Cloning into ", C.muted), T("./gitmap-v13", C.path), T(" (default branch: ", C.muted), T("main", C.flag), T(")", C.muted)),
  ...
  // 3) history
  {
    ...
    cwd: "~/repos/gitmap-v13",
    lines: [
      cmd("~/repos/gitmap-v13", T("gitmap ", C.command), T("history ", C.brandGold), T("--limit 5", C.flag)),
  ...
  // 4) release
  {
    ...
    cwd: "~/repos/gitmap-v13",
    lines: [
      cmd("~/repos/gitmap-v13", T("gitmap ", C.command), T("release ", C.brandGold), T("--bump minor", C.flag)),
