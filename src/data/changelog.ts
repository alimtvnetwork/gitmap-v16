...
export const changelog: ChangelogEntry[] = [
  {
    ...
    items: [
      ...
      "Constants centralized in `gitmap/constants/constants_fixrepo.go` per the no-magic-strings rule: `FixRepoExitTestsFailed = 9`, `FixRepoFlagStrict = \"strict\"`, plus 7 message format strings (`FixRepoMsgStrictSkipDryRun`, `…NoGoFiles`, `…NoPackages`, `…RunFmt`, `…PassFmt`, `FixRepoErrStrictMissing`, `FixRepoErrStrictFailFmt`). Help system updated: `HelpFRStrict` line added in `constants_fixrepohelp.go`; `HelpFixRepoExitCodes` extended with `9 tests-failed`; `rootusageflags.go::printUsageFixRepoFlags` prints the new line; `gitmap/helptext/fix-repo.md` documents the synopsis, behavior step 6, and the new exit code.",
      "Tests: new `gitmap/cmd/fixrepo_strict_packages_test.go` pins five contracts on the pure derivation helper — empty-input → nil; root + subdirs → correct `.` and `./<dir>` patterns; outside-root paths silently skipped; Windows backslashes normalized to forward slashes (Windows-only branch); deterministic sort across 20 repeat calls (defends against the map-iteration randomization that would otherwise flake CI). The orchestration layer (`runFixRepoStrict`, `invokeGoTest`) is intentionally untested at the unit level because shelling out to `go test` is the integration the strict mode IS — exercised by the next `gitmap fix-repo --strict` run in CI.",
      "Files: `gitmap/cmd/fixrepo.go` (option field + dispatcher exit-9 branch), `gitmap/cmd/fixrepo_flags.go` (parser + `isFixRepoStrictArg`), `gitmap/cmd/fixrepo_strict.go` (new), `gitmap/cmd/fixrepo_strict_packages.go` (new), `gitmap/cmd/fixrepo_strict_packages_test.go` (new), `gitmap/cmd/rootusageflags.go` (help line wiring), `gitmap/constants/constants_fixrepo.go` (exit code + flag + 7 messages), `gitmap/constants/constants_fixrepohelp.go` (help strings), `gitmap/helptext/fix-repo.md` (synopsis + behavior + exit codes), `fix-repo.ps1` (synopsis + parser + `Invoke-PostRewriteStrict` + main flow), `src/constants/index.ts` (VERSION → v4.14.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.13.0",
  ...
  },
  {
    ...
    items: [
      "Closed a long-standing test desync in `gitmap/cmd/replaceversionparse_test.go::TestPairsForTarget` and `gitmap/cmd/replaceaudit_test.go::TestBuildAuditNeedles`. Both tests passed `current=9` (or implied a 1-digit current) into the function under test, but their hard-coded `want` slices included the literal string `\"gitmap-v13\"` next to a `\"gitmap/v9\"` neighbor — an internally contradictory expectation that drifted whenever the project's own version was bumped past v9. After every width-crossing release the same 4-line edit had to be re-applied by hand; if the edit was missed, the test silently locked in the wrong contract.",
  ...
  },
  {
    ...
    items: [
      ...
      "File: `gitmap/cmd/clone.go` — one `WriteShellHandoff(absPath)` call inside `executeDirectClone`, placed after the DB upsert + Desktop registration but before the VS Code open so the cd happens regardless of whether VS Code is installed.",
    ],
  },
  {
    version: "v3.117.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Renamed `gitmap-v3` → `gitmap-v13` across Go constants, install/uninstall scripts, spec docs, helptext, README, React data files, and GitHub workflows. Only `.gitmap/` artifacts retain v3 references (immutable per project policy).",
  ...
  },
  {
    ...
    items: [
      ...
      "Fixed `gitmap/store/migrations.go` build error caused by an orphan `\"github.com/alimtvnetwork/gitmap-v13/gitmap/constants\"` import.",
  ...
  },
  {
    ...
    items: [
      ...
      "Repository renamed from `git-repo-navigator` to `gitmap-v13`.",
    ],
  },
  {
    version: "v2.51.0",
